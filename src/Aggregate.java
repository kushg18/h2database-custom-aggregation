...
public class Aggregate extends AbstractAggregate {
    ...
    public enum AggregateType {
         ...
         * Aggregate type to get the PIVOT OF LIST, PIVOT(expression)
         */
        PIVOT,
    }
    ...
    static {
        ...
        // User defined
        addAggregate("PIVOT", AggregateType.PIVOT);
    }
    ...
    private Value getValueQuick(Session session) {
        ...
        case PIVOT:
        	System.out.println("In get value quick!!");
        	Table pTable = select.getTopTableFilter().getTable();
	       	ValueLong pCount= ValueLong.get(pTable.getRowCount(session));
	       	int tempLimit = pCount.getInt() / 2;
	       	int plimit = tempLimit != 0 ? tempLimit + 1 : tempLimit;
	       	Index pIndex = getMinMaxColumnIndex();
	       	Cursor pCursor = pIndex.find(session, null, null);
	       	int index = 0;
	       	while (index < plimit) {
	       		pCursor.next();  
	       		index++;
	       	}
	       	SearchRow pRow = pCursor.getSearchRow();
	       	if (pRow == null) {
	       		return ValueNull.INSTANCE;
	       	}
	       	else {
	       		int indexValue = pIndex.getColumns()[0].getColumnId();
	       		return pRow.getValue(indexValue);
	       	}
        default:
    ...
    public boolean isEverything(ExpressionVisitor visitor) {
        ...
        if (visitor.getType() == ExpressionVisitor.OPTIMIZABLE_MIN_MAX_COUNT_ALL) {
            ...
            case MAX:
            case PIVOT:
            	System.out.println("In isEverything!!");
                Index index = getMinMaxColumnIndex();
                return index != null;
            /** 
